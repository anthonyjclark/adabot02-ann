...
int main(int argc, char const *argv[])
{
    ...
    while (world->getTime() < TIME_STOP + TIME_STEP/2.0) {
        ...
        if (world->getTime() >= next_control_update_time) {
            ...
            double angle_norm = angle / 1_pi;

            // left_speed = max(-max_rads, min(max_rads, fsm[state].left_speed));
            // right_speed = max(-max_rads, min(max_rads, fsm[state].right_speed));

            if (angle_norm > 0) {
                left_speed = -max_rads * (((1 - abs(angle_norm)) * 2) - 1);
                right_speed = -max_rads;
            } else {
                left_speed = -max_rads;
                right_speed = -max_rads * (((1 - abs(angle_norm)) * 2) - 1);
            }

            // left_speed = max_rads / 10;
            // right_speed = -max_rads / 10;
